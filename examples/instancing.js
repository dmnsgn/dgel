...
const pipeline = new Pipeline({
  ...
  ins: [
    WGSLBuiltIn.InstanceIndex,
    new Attribute("position", "vec3"),
    new Attribute("normal", "vec3"),
    new Attribute("uv", "vec2"),
  ],
  outs: [WGSLBuiltIn.Position, new Attribute("vNormal", "vec3")],

  // WGSL
  vertexBody: /* wgsl */ `${Shaders.NOISE.CLASSIC.CNOISE3D}`,
  vertex: /* wgsl */ `
  var output: Output;
  ...
  let displacement = custom.amplitude * cnoise3d(custom.frequency * worldPosition.xyz + custom.time);
  let offset = vec3<f32>(worldPosition.x, worldPosition.y + displacement, worldPosition.z);

  output.position = system.projectionMatrix * system.viewMatrix * modelMatrix * vec4<f32>(position + offset, 1.0);
...
`,
  ...
  glsl: {
    language: "glsl",
    vertexBody: /* wgsl */ `${Shaders.GLSL.NOISE.CLASSIC.CNOISE3D}`,
    vertex: /* glsl */ `
  vNormal = normal;
