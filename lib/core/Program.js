...
class Program {
  ...
  getGLSLHeaders(set, entries) {
    ...
    for (let i = 0; i < entries.length; i++) {
      var _binding$qualifiers;

      const binding = entries[i];
      const layoutQualifierString = ((_binding$qualifiers = binding.qualifiers) === null || _binding$qualifiers === void 0 ? void 0 : _binding$qualifiers.layout) ? `${binding.qualifiers.layout || ""}, ` : ""; // uniform-buffer
