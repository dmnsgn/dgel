...
const DEFAULT_GLSL_FRAGMENT_OUT_COLOR = [
...
class Pipeline {
    ...
    init() {
        ...
        if (this.ins?.length > 0) {
            hasArrayOfAttributes = this.ins.some((variable) => variable instanceof Attribute);
        ...
        if (this.compute) {
            this.program = new Program(this.bindGroupLayouts, {
                compute: new Shader({
                ...
                ? [
                    {
                        ...
                        arrayStride: this.ins
                            ...
                            .reduce((a, b) => a + b, 0),
                        attributes: mapAttributes(this.ins.filter((variable) => variable instanceof Attribute)),
                    },
                ]
